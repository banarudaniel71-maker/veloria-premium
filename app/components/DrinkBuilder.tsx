...
type ShotPack = {
  ...
  meta: string;
  tag?: string;
  picks: number; // câte shot-sorten alege clientul
};
...
function SectionCard({
  ...
  return (
    <div className="rounded-3xl border border-white/12 bg-gradient-to-b from-white/12 to-white/5 p-6 backdrop-blur-md shadow-2xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-2 text-sm text-white/70">
            Wählt <span className="text-white font-semibold">{limit}</span> aus.{" "}
...
export default function DrinkBuilder() {
  ...
  const pack = useMemo(() => SHOT_PACKS.find((p) => p.id === shotPack) ?? null, [shotPack]);
  const picksRequired = pack?.picks ?? 0;

  // când schimbi pachetul, reset shot sorten
  ...
  const completion = useMemo(() => {
    // total: 6 + 3 + 1 + picksRequired (2 sau 3)
    const total = 10 + (picksRequired || 0);
    const done = cocktails.length + mocktails.length + (pack ? 1 : 0) + shotFlavors.length;
    return clamp01(total > 0 ? done / total : 0);
  }, [cocktails.length, mocktails.length, pack, shotFlavors.length, picksRequired]);

  const summaryText = useMemo(() => {
    const c = cocktails.map((id) => COCKTAILS.find((x) => x.id === id)?.name).filter(Boolean).join(", ");
    const m = mocktails.map((id) => MOCKTAILS.find((x) => x.id === id)?.name).filter(Boolean).join(", ");
    const s = pack?.title ?? "-";
    const sf = shotFlavors.map((id) => SHOT_FLAVORS.find((x) => x.id === id)?.name).filter(Boolean).join(", ");
    return `Cocktails (6): ${c || "-"}\nMocktails (3): ${m || "-"}\nShot Paket: ${s}\nShot Sorten (${picksRequired || 0}): ${sf || "-"}`;
  }, [cocktails, mocktails, pack, shotFlavors, picksRequired]);

  // fără email de business -> trimitem selecția către /kontakt ca query param
    ...
    <div className="relative">
      ...
      <div className="mb-6 rounded-3xl border border-white/12 bg-white/5 p-5 backdrop-blur-md">
        <div className="flex items-center justify-between gap-4">
          <div>
            <div className="text-sm font-semibold text-white">Auswahl Builder</div>
            <div className="mt-1 text-xs text-white/65">
              {ready
                ? "Auswahl komplett. Jetzt Anfrage senden."
                : "Wähle 6 Cocktails, 3 Mocktails, 1 Shot Paket und die Shot Sorten."}
            </div>
          </div>

          <div className="text-xs text-white/70 rounded-2xl border border-white/10 bg-black/20 px-3 py-2">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-3xl border border-white/12 bg-gradient-to-b from-white/12 to-white/5 p-6 backdrop-blur-md shadow-2xl">
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-xs text-white/55">{active ? "Selected" : "Pick"}</div>
                  </div>
                </motion.button>
              );
            })}
          ...
          {/* Shot sorten selector (după ce aleg pachet) */}
          <AnimatePresence initial={false}>
            {pack ? (
              <motion.div
                key="shot-sorten"
                ...
                className="mt-6"
              >
                <div className="text-sm font-semibold text-white">Shot Sorten auswählen</div>
                <div className="mt-1 text-xs text-white/65">
                  Wähle <span className="text-white font-semibold">{picksRequired}</span> Sorten aus.
                </div>
                  ...
                  {SHOT_FLAVORS.map((sf) => {
                    ...
                    return (
                      <Chip
                        ...
                        onClick={() => {
                          if (active) {
                            setShotFlavors(shotFlavors.filter((x) => x !== sf.id));
                            return;
                          }
                ...
                <div className="mt-4 rounded-2xl border border-white/10 bg-black/15 p-4">
                  ...
                  <div className="mt-2 space-y-2">
                    {shotFlavors.length === 0 ? (
                      <div className="text-sm text-white/60">Noch nichts ausgewählt.</div>
                    ) : (
                      shotFlavors.map((id) => {
          ...
          <div className="mt-6 rounded-2xl border border-white/10 bg-black/15 p-4 text-xs text-white/65">
            Wenn ihr unsicher seid, empfehlen wir euch die beste Kombination passend zu Event & Gästen.
          </div>
        </div>
      </div>
      ...
      <div className="mt-10 rounded-3xl border border-white/12 bg-white/5 p-6 backdrop-blur-md">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="text-sm font-semibold text-white">Zusammenfassung</div>
            <div className="mt-1 text-xs text-white/65">Diese Auswahl könnt ihr direkt an uns schicken.</div>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <Link
              ...
              className={`inline-flex items-center justify-center rounded-2xl px-5 py-3 text-sm font-semibold transition ${
                ready ? "bg-white text-black" : "bg-white/20 text-white/60 cursor-not-allowed"
              }`}
            >
              Auswahl anfragen
            </Link>
            ...
            <Link
              href="/kontakt"
              className="inline-flex items-center justify-center rounded-2xl border border-white/15 bg-white/5 px-5 py-3 text-sm font-semibold text-white backdrop-blur hover:bg-white/10"
            >
              Kontakt öffnen
            </Link>
          </div>
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4">
          <pre className="whitespace-pre-wrap text-xs text-white/80">{summaryText}</pre>
        </div>
      </div>
    </div>
  );
